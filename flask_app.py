...
def identify_and_process_files(files):
    ...
    for file in files:
        ...
        try:
            ...
            # ※日本語文字化け対策で encoding='utf-8_sig' を指定（必要に応じて 'shift_jis' に変更）
            try:
        ...
        if not API_KEY:
            ...
            if not txt_chrono or not txt_sns:
                ...
                try:
                    ...
                    【照合ルール（重要）】
                    1. 時間軸のズレ: SNS投稿はシステム登録日時よりも「前（数分〜半日前）」であることが多い。前後12時間の幅で許容する。
                    2. 場所の特定: 「西区」「東区」といった広域だけでなく、「町名（愛宕、千早など）」や「ランドマーク（博多駅など）」の一致を最重視する。
                    3. 事象の類似: 「道路冠水」と「水没」、「崖崩れ」と「土砂」など、表現が異なっても意味が通じる場合は一致とみなす。
                    4. 確信度のスコアリング: 0%（無関係）〜100%（間違いなく同一）で評価。
                       - 100%: 日時・詳細地名・事象すべてが完全に一致。
                       - 80%: 詳細地名と事象は一致するが、日時のズレが大きい。
                       - 60%: エリア（区レベル）と事象は一致するが、詳細な場所が不明確。
                       - 40%以下: 偶然の一致の可能性が高いもの。
                    ...
                    【出力形式 (JSON)】
                    結果をUIで表示するため、必ず以下のJSON配列形式のみを出力してください。
                    Markdownの装飾や解説文は一切不要です。JSONデータのみを返してください。
                    
                    [
                        {{
                            "match_id": 1,
                            "score": 95,
                            "reason": "マッチングした理由を簡潔に記述",
                            "location_label": "表示用の地名 (例: 博多駅前)",
                            "lat": 33.590, 
                            "lng": 130.412,
                            "event_type": "事象の種類 (例: 道路冠水)",
                            "chrono_info": "システム側のIDと内容の要約",
                            "sns_info": "SNS側の日時とテキストの要約"
                        }},
                        ...
                    ]
                    """
